...
compile:
	...
	@p4c-bmv2 src/FlexMesh.p4 --json build/FlexMesh.json

run: compile
	@cp build/FlexMesh.json $(SWITCH_DIR)
	@cd $(SWITCH_DIR)&&sudo bash simple_switch FlexMesh.json $(INTF) $(LOG)
...
run-fattree:
	...
	@make compile
	@cp build/FlexMesh.json $(SWITCH_DIR)
	@cp build/FlexMesh.json $(SWITCH_DBG_DIR)
